...
export function SidebarHistory() { // 移除user prop
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<Array<Chat>>('/api/history', fetcher, { // 始终请求数据
    fallbackData: [],
  });
  ...
  const groupChatsByDate = (chats: Chat[]): GroupedChats => {
    ...
    return chats.reduce((groups, chat) => {
      ...
      else if (chatDate > subWeeks(now, 1)) groups.lastWeek.push(chat);
      else if (chatDate > subMonths(now, 1)) groups.lastMonth.push(chat);
        ...
        <SidebarGroupContent>
          <SidebarMenu>
            {history && (() => {
              ...
              return Object.entries(groupedChats).map(([period, chats]) => (
                chats.length > 0 && (
                  <div key={period}>
                    <div className="px-2 py-1 text-xs text-sidebar-foreground/50 mt-6">
                      {period.replace(/([A-Z])/g, ' $1').replace(/^./, s => s.toUpperCase())}
                    </div>
                    {chats.map(chat => (
                      <ChatItem
                        key={chat.id}
